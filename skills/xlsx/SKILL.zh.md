---
name: xlsx
description: "支持公式、格式化、数据分析和可视化的全面电子表格创建、编辑和分析。当Claude需要使用电子表格（.xlsx、.xlsm、.csv、.tsv等）进行：(1) 创建带公式和格式的新电子表格，(2) 读取或分析数据，(3) 修改现有电子表格同时保留公式，(4) 电子表格中的数据分析和可视化，或(5) 重新计算公式时"
...
### 零公式错误
- 每个Excel模型*必须*交付零公式错误（#REF!、#DIV/0!、#VALUE!、#N/A、#NAME?）

### 保留现有模板（更新模板时）
- 在修改文件时研究和精确匹配现有格式、样式和约定
...
#### 硬编码文档要求
- 在单元格内或旁边（如果表尾）注释或格式化："来源：[系统/文档]、[日期]、[特定引用]、[URL（如果适用）]"
- 示例：
...
用户可能要求您创建、编辑或分析.xlsx文件的内容。您有不同工具和工作流程可用于不同任务。

## 重要要求

**LibreOffice需要用于公式重新计算**：您可以假设LibreOffice已安装，用于使用`recalc.py`脚本重新计算公式值。脚本在首次运行时自动配置LibreOffice
...
6. **验证并修复任何错误**：
   - 脚本返回包含错误详情的JSON
   - 如果`status`是`errors_found`，检查`error_summary`了解特定错误类型和位置
   - 修复识别的错误并重新计算
   - 要修复的常见错误：
     - `#REF!`：无效单元格引用
     - `#DIV/0!`：除零
...
sheet = wb.active  # 或特定工作表的wb['SheetName']
...
由openpyxl创建或修改的Excel文件包含字符串形式的公式但不包含计算值。使用提供的`recalc.py`脚本重新计算公式：
